...
export const indexDataset = action({
  ...
  handler: async (
    ctx,
    { ckanId, title, notes, tags, organizationId, organizationTitle }
  ) => {
    // Build searchable text from title, notes, and tags
    // Tags are included in text for semantic search
    const searchableText = [title, notes || "", ...tags]
      .filter(Boolean)
      .join(" ");
  ...
  handler: async (ctx, { datasets }) => {
    ...
    for (const dataset of datasets) {
      try {
        ...
        errors.push(
          `${dataset.ckanId}: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
