...
export function MessageItem({
  ...
  const hasActiveTools = toolParts.some(
    ({ part }) => isToolPart(part) && getToolStatus(part.state) === 'active'
  );
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 flex flex-col gap-6 duration-300">
      ...
      {message.parts.map((part, i) => {
        switch (part.type) {
          case 'text': {
            const isLastPart =
              i === message.parts.length - 1 && isLastMessage;
          ...
          case 'reasoning': {
            ...
            return (
              <ReasoningPart
                key={`${message.id}-${i}`}
                isCurrentlyReasoning={isCurrentlyReasoning}
              />
            );
          }
          default:
