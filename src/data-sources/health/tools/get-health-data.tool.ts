...
import { buildHealthDataUrl } from '../api/health.endpoints';
...
export const resolveSourceUrl: ToolSourceResolver = (input, output) => {
    ...
    const name = getString(input, 'searchedResourceName') ?? getString(output, 'endPointName');
    return {
        url: apiUrl,
        title: name ? `נתוני בריאות — ${name}` : 'נתוני בריאות',
        urlType: 'api',
    };
};
